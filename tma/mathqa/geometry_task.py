    ...
    def enumerate_task_plans(self, task_store: TaskStore):
        ...
        for param_count, templates in template_breakdown.items():
            ...
            for template in tqdm(
                templates, desc=f"Enumerating templates with {param_count} params"
            ):
...
class MidpointGenerator(GeoPlanGenerator):
    ...
    def enumerate_task_plans(self, task_store: TaskStore):
        ...
        for templates in template_breakdown.items():
            for template in tqdm(
                templates, desc="Enumerating templates for midpoint tasks"
            ):
                for x1 in x_values:
                    for y1 in y_values:
                        for z1 in z_values:
...
class IntersectionGenerator(GeoPlanGenerator):
    ...
    def enumerate_task_plans(self, task_store: TaskStore):
        ...
        for param_count, templates in template_breakdown.items():
            for template in tqdm(
                templates, desc="Enumerating templates for intersection tasks"
            ):
                for x1 in x_values:
                    for y1 in y_values:
                        for x2 in x_values:
...
class CircleGenerator(GeoPlanGenerator):
    ...
    def enumerate_task_plans(self, task_store: TaskStore):
        ...
        for num_paras, texts in template_breakdown.items():
            for text in texts:
                for radius in tqdm(
                    all_radius, desc=f"Enumerating templates with {num_paras} params"
                ):
...
class AngleGenerator(GeoPlanGenerator):
    schema = {"question_template": "str", "angles": "list"}  # Store list of angles
